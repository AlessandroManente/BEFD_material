...
All the variables have seasonality and different trends. Japan has a strongly NON-linear behavior, instead for New Zealand is LINEAR. 
We focus on Japan and New Zealand because they star in the same way but then from 2001 they have opposite trends. We can hypothesize a competitive behavior for the two countries
...
We have two ANOVA: for parametric and NON-paramtric effects. For seas we don't have a non-par. part because we don't model it with splines (it is pointless modelling the seasonality with smoothing splines because is a factorial variable).
...
We compare the linear model m2 (obtained by stepwise selection) and g3 (considering the AIC).
We do NOT compare m1 and g3, m1 is the starting point in the selection procedure of g3, so we definitely find a better model.
...
Model with all the variables except "vote_classes" (highly correlated with our response); now we have a numerical response, with a categorical one we have to change the distribution.
...
The acf of residuals is very satisfactory (aside from the first autocorrelation) we will deal with a residual situation with constant mean(=0) and constatn variance.
